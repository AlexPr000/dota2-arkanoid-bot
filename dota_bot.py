        ...
        if self.is_running:
            try:
                ...
                if box_w > 10 and box_h > 10:
                    ...
                    # 2. Поиск Вагонетки (МОНОЛИТНЫЙ ГЕОМЕТРИЧЕСКИЙ ДЕТЕКТ)
                    ...
                    # Склеиваем микро-трещины и дыры в маске, чтобы края не мерцали
                    kernel = np.ones((5, 5), np.uint8)
                    mask_cleaned = cv2.morphologyEx(mask_red, cv2.MORPH_CLOSE, kernel)

                    # Находим координаты СРАЗУ ВСЕХ красных точек в желтой зоне
                    pts = cv2.findNonZero(mask_cleaned)

                    wagon_x, wagon_y = None, None
                    if pts is not None:
                        # Строим ОДИН общий прямоугольник, обводящий абсолютно весь красный цвет
                        x, y, w, h = cv2.boundingRect(pts)
                        
                        # Фильтр от случайных искр и бликов (вагонетка явно шире 30 пикселей)
                        if w > 30 and h > 5:
                            # Центр теперь ВСЕГДА будет железно посредине кареты
                            wagon_x = int(x + (w / 2))
                            wagon_y = int(y + (h * 1.7)) + split_y_rel
